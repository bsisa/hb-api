...
/**
 * Example code StreamingBodyParser. Original at: 
 * https://github.com/mslinn/play21-file-upload-streaming/blob/master/app/services/StreamingBodyParser.scala 
...
object StreamingBodyParser {
  ...
  def streamingBodyParser(streamConstructor: String => Option[OutputStream]) = BodyParser { request =>
    ...
    Logger.debug(s"StreamingBodyParser.streamingBodyParser(${streamConstructor}) object called.")
  ...
  def streamingFilePartHandler(request: RequestHeader): PartHandler[FilePart[Either[StreamingError, StreamingSuccess]]] = {
    parse.Multipart.handleFilePart {
      case parse.Multipart.FileInfo(partName, filename, contentType) =>
        ...
        Logger.debug(s"StreamingBodyParser.streamingFilePartHandler case parse.Multipart.FileInfo(partName = ${partName}, filename = ${filename}, contentType = ${contentType}) called.")
        
        // Reference to hold the error message
        var errorMsg: Option[StreamingError] = None

        /* Create the output stream. 
         * If something goes wrong while trying to instantiate the output stream, assign the
         * error message to the result reference, e.g. `result = Some(StreamingError("network error"))`
         * and set the outputStream reference to `None`; the `Iteratee` will then do nothing and 
         * the error message will be passed to the `Action`. */
