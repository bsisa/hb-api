...
/**
 * Tests HospitalHelperForPrtSpec
 *
 * Warning: `Simulation` test MUST be sequential to behave correctly
 *
 * Tip: from sbt play console run:
 * {{{
 * test-only test.ch.bsisa.hyperbird.patman.simulations.model.HospitalHelperForPrtSpec
 * }}}
 * to have only the current test run.
 *
 * @author Patrick Refondini
 */
class HospitalHelperForPrtSpec extends Specification {
  // Set sequential execution
  sequential

  val TestResourcesDir = "./test/resources/"

  val EXPECTED_PRT_CODE = "prt"
  ...
  val hospitalState1ElfinFile = "HOSPITAL_STATE_1_PRT.xml"
  val hospitalState2ElfinFile = "HOSPITAL_STATE_2_PRT.xml"
  val hospitalState3ElfinFile = "HOSPITAL_STATE_3_PRT.xml"
  val hospitalState4ElfinFile = "HOSPITAL_STATE_4_PRT.xml"
  val hospitalState5ElfinFile = "HOSPITAL_STATE_5_PRT.xml"
  val hospitalState6ElfinFile = "HOSPITAL_STATE_6_PRT.xml"
  val hospitalState7ElfinFile = "HOSPITAL_STATE_7_PRT.xml"
  val hospitalState8ElfinFile = "HOSPITAL_STATE_8_PRT.xml"
  val hospitalState9ElfinFile = "HOSPITAL_STATE_9_PRT.xml"
  ...
  "The hospitalState1" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState1.code mustEqual EXPECTED_PRT_CODE
    }
    ...
    s"have 4 busy beds" in {
      hospitalState1.beds.filterNot(b => b.free) must have size (4)
    }
    s"have 6 free beds" in {
      hospitalState1.beds.filter(b => b.free) must have size (6)
    }
  }

  "The hospitalState2" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState2.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState3" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState3.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState4" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState4.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState5" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState5.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState6" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState6.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState7" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState7.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState8" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState8.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "The hospitalState9" should {
    s"have code '${EXPECTED_PRT_CODE}'" in {
      hospitalState9.code mustEqual EXPECTED_PRT_CODE
    }
  ...
  "Simulation" should {
    ...
    s"Simulated HS t=1 must contain 4 SC beds" in {
      simulatedHospitalState.get.beds.filter(b => b.patientType == Constants.PATIENT_TYPE_SC) must have size 4
    }

    s"HospitalState 1 provides 0 incoming SI beds" in {
      bedsWithIncomingPatientTypeSiVar must have size 0
    }
    s"HospitalState 1 provides 4 incoming SC bed" in {
      bedsWithIncomingPatientTypeScVar must have size 4
    }
    ...
    s"HospitalState 1 after CDF TransferCreate processing must contain 2 SI beds" in {    
    // Simulate CDF TransferCreate 
    // Sum up CDF incoming SI and CDF patient type change from SC to SI which are all transferred here at PRT
    val bedsWithIncomingPatientTypeSiFromCdf: List[Bed] = List(
      Bed(id = "503A", free = false, patientNb = "9995826", patientType = "soins intensifs", transferType = "spécialisé"),
      Bed(id = "505", free = false, patientNb = "9991663", patientType = "soins intensifs", transferType = "médicalisé"))
      
    val patientTypeChangeFromScToSiFromCdf: List[Bed] = List()

    val incomingTransferredSi = bedsWithIncomingPatientTypeSiFromCdf ++ patientTypeChangeFromScToSiFromCdf

    simulationSummary = Some(
      HospitalHelper.updateHospitalSimulationSummary(
        hospitalCode = EXPECTED_PRT_CODE,
  ...
  def updateVarsForStep() = {
    HospitalHelper.getBedsUpdates(previousHospitalState, currentHospitalState) match {
      case (
        ...
        // ================================================================================================================================= 
        // Update current PRT simulatedHospitalState 
        // =================================================================================================================================
        simulatedHospitalState = HospitalHelper.updateSimulatedHospitalStateForPrt(
          currentSimulatedHospitalStateOption = simulatedHospitalState,
          newStaticHospitalStateOption = currentHospitalState,
